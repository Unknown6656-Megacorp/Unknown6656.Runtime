...
#warning TODO : nullability and tuple names
    ...
    protected override string GetValueRepresentation(object? value, Type type) => value switch
    {
        ...
        string x => $"\"{x.Select(s => s.ToCSharpEscaped(false)).StringConcat()}\"",
    ...
    protected (List<string> attributes, string signature) GenerateSignature(MethodBase method)
    {
        ...
        CallingConventions x = method.CallingConvention;
        MethodImplAttributes y = method.MethodImplementationFlags;
        ////////////////////////////// TODO //////////////////////////////
    ...
    public override string GenerateSignature(MemberInfo member)
    {
        ...
        switch (member)
        {
            ...
            case MethodBase method:
                ...
                break;
            case EventInfo ei:
                MethodInfo? add = ei.AddMethod;
                MethodInfo? rem = ei.RemoveMethod;

                throw new NotImplementedException();
            case PropertyInfo pi:
                MethodInfo? set = pi.SetMethod;
                MethodInfo? get = pi.GetMethod;
